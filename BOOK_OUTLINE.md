...
正文不只给出编译命令和文件路径：每个实验的完整 `model.xml`、`main.cc` 和 `CMakeLists.txt` 都就地嵌入对应知识点的讲解中。读者可以在不离开章节的情况下逐行阅读，再进入示例目录直接运行同一份代码。
...
每个实验还配有真实 MuJoCo 原生渲染效果图。建模类实验通过官方 `simulate` 交互观察；必须显示自定义控制或估计算法状态的实验，则在单个 `main.cc` 中直接展开最小 GLFW + `mjv/mjr` viewer，让可视化管线也成为可学习的正文。
...
机械臂项目将模型审计、DLS、质量矩阵、偏置力、力矩限幅和任务验收组合起来；Unitree G1 项目则在开源 29-DoF 人形模型上组合 free-joint 状态、keyframe、整机 CoM、8 点足底 wrench、支撑区域和站立验收。
